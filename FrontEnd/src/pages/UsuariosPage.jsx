...
export const UsuariosPage = () => {
  ...
  const GetPerfil = async () => {
    ...
    const perfilesFiltrados = resp.filter((perf) => !perf.deleted);
  ...
  const peticionPost = async () => {
    const usuarioRepetido = usuarios.filter(
      (usuario) => usuario.usuario === usuarioSeleccionado.usuario
    );
    ...
    if (
      usuarioSeleccionado.password === usuarioSeleccionado.repetir_contraseña
    ) {
    ...
    if (
      usuarioRepetido.length === 0 &&
      usuarioSeleccionado.idPerfil !== 0 &&
      usuarioSeleccionado.password.length >= 4 &&
      ...
      await postUsuarios(usuarioSeleccionado);
  ...
  const peticionDelete = async () => {
    ...
    while (i < UsuarioEliminar.length) {
      await deleteUsuarios(UsuarioEliminar[i]);
  ...
  const handleSelectRow = (ids) => {
    if (ids.length > 0) {
      setUsuarioSeleccionado(
        usuarios.filter((usuario) => usuario.id === ids[0])[0]
      );
    } else {
  ...
  const clientesUnicos = clientes.filter(
    (cliente, index, self) =>
              ...
              <Card>
                <DataGrid
                  ...
                  onRowClick={(usuarioSeleccionado, evt) => {
                    ...
                    setPerfilUsuarioEditar(
                      perfiles.filter(
                        (perfil) => perfil.id === usuarioParaEditar.idPerfil
                      )
                    );
                    setClienteUsuarioEditar(
                      clientes.filter(
                        (cliente) =>
                          cliente.codigo === usuarioParaEditar.idCliente
                      )
                    );
                    setestadoCboCliente(
                      usuarioParaEditar.idPerfil !== 2 &&
                      usuarioParaEditar.idPerfil !== 4
                    );
            ...
            <ModalLayout
              ...
              contenido={
                <EditarUsuarioModal
                  ...
                  change={handleChange}
                  handleChangeContraseña={handleChangeContraseña}
                  handleChangePerfil={handleChangePerfil}
                  estadoCliente={estadoCboCliente}
            ...
            <ModalLayout
              ...
              contenido={
                rowsIds.length > 1 ? (
                  <Grid item xs={12}>
                    <Typography>
                      ¿Estás seguro que deseas eliminar los <b>{rowsIds.length}</b> usuarios seleccionados?
                    </Typography>
                  </Grid>
                ) : (
                  <>
                ...
                <DataGrid
                  ...
                  onRowClick={(usuarioSeleccionado, evt) => {
                    ...
                    setPerfilUsuarioEditar(
                      perfiles.filter(
                        (perfil) =>
                          perfil.id === usuarioSeleccionado.row.idPerfil
                      )
                    );
                    setClienteUsuarioEditar(
                      clientes.filter(
                        (cliente) =>
                          cliente.codigo === usuarioSeleccionado.row.idCliente
                      )
                    );
                    if (usuarioSeleccionado.row.idPerfil === 2) {
                      setestadoCboCliente(false);
                    } else {
                      setestadoCboCliente(true);
                    }
