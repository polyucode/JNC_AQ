...
export const PintarCarpeta = ({
  ...
  const [mensajeBotonAceptarVentanaModal, setMensajeBotonAceptarVentanaModal] =
    useState("");
  ...
  const [visualizarIntroducirTextoModal, setVisualizarIntroducirTextoModal] =
    useState(true);
  ...
  const handleClickEditarNombreCarpeta = (carpeta) => {
    if (carpeta.nombre === carpetaRaiz) {
      mostrarNotificacion(
        "Error",
        "No se puede modificar la carpeta principal"
      );
      return;
    }
  ...
  const handleClickEliminarCarpeta = (carpeta) => {
    ...
    setDescripcionTextoModal(
      "Esta acción es irreversible. Se eliminarán todos los elementos contenidos en ella. ¿Desea continuar?"
    );
  ...
  const handleClickEliminarFichero = (carpeta, nombreFichero) => {
    ...
    setMensajeBotonAceptarVentanaModal("Eliminar");
    setDescripcionTextoModal(
      `Eliminar ${nombreFichero} Esta acción es irreversible ¿Desea continuar?`
    );
    setVisualizarIntroducirTextoModal(false);
    setCarpetaSeleccionada(carpeta);
  ...
  return (
    <div className="estructura-carpetas">
      <button
        ref={buttonActualizar}
        onClick={() => enviarActualizar()}
        style={{ display: "none" }}
      ></button>
      <div className="contenedor-carpeta">
        <IconButton onClick={() => setMostrarContenido(!mostrarContenido)}>
          <ExpandMoreIcon
            style={{
              transform: mostrarContenido ? "rotate(180deg)" : "rotate(0deg)",
              transition: "0.3s",
            }}
          />
        </IconButton>
        ...
        {permisoUsuario === 1 && (
          <div className="botones-carpeta">
            <IconButton
              onClick={(event) => handleClickEditarNombreCarpeta(carpeta)}
            >
              <EditIcon />
            </IconButton>
            ...
            <IconButton
              onClick={(event) => handleClickEliminarCarpeta(carpeta)}
            >
              <DeleteIcon />
            </IconButton>
            <IconButton
              onClick={(event) => handleClickSubirFicheroCarpeta(carpeta)}
            >
      ...
      {/* Lista de ficheros, visible solo si mostrarArchivos es true */}
      {mostrarContenido && (
        <>
            ...
            {carpeta.ficheros?.map((nombreFichero) => (
              <div className="fichero-row" key={nombreFichero}>
                ...
                {permisoUsuario === 1 && (
                  <IconButton
                    onClick={() =>
                      handleClickEliminarFichero(carpeta, nombreFichero)
                    }
                  >
                    <DeleteIcon />
                  </IconButton>
                ...
                <IconButton
                  onClick={() =>
                    handleClickDescargarFichero(carpeta, nombreFichero)
                  }
                >
          ...
          <div
            className="contenedor-subcarpetas"
      ...
      <ModalLayout
        ...
        botones={[
          insertarBotonesModal(
            ...
            mensajeBotonAceptarVentanaModal,
            async () => {
              confirmarAccion();
            }
          ),
